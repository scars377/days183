	...
	render: function() {
		return (
			<div>
				...
				<textarea className='paste-input' ref="import" onChange= {this.importFromText}/>
				...
				<div className='inputs'>
					...
					<button className={'dep ' + (this.state.dep
						? 'active'
						: 'deactive')} onClick={this.setDep(true)}>
						出境
					</button>

					<button className={'arr ' + (!this.state.dep
						? 'active'
						: 'deactive')} onClick={this.setDep(false)}>
						入境
					</button>

					<input type='text' ref='input' placeholder='輸入日期' value={this.state.input} onKeyDown={this.inputDown} onChange={this.inputChange}/>

					<button className={this.state.enabled
						? 'enabled'
